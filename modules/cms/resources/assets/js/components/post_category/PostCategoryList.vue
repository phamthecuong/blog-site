<template>
    <section class="content">
                                ...
                                <div class="form-group m-0">
                                    <div class="form-line">
                                        ...
                                        <input type="text" class="form-control" name="name" v-on:keyup="searchEvent"
                                               placeholder="Nhập danh mục bài viết" value=""/>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="body table-responsive">
                            <table class="table table-striped">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span v-if="category.is_active == 1" class="font-bold col-teal">Hoạt động</span>
                                        <span v-else class="font-bold col-pink">Không hoạt động</span>
                                    </td>
                                    <td class="text-right">
                                        <router-link class="item-tool" :to="{path: '/post-category/'+category.id}">
                                ...
                                <tr v-for="sub_category in category.categories">
                                    ...
                                    <td>
                                        <span v-if="sub_category.is_active == 1"
                                              class="font-bold col-teal">Hoạt động</span>
                                        <span v-else class="font-bold col-pink">Không hoạt động</span>
                                    </td>
                                    <td class="text-right">
                                        <router-link class="item-tool" :to="{path: '/post-category/'+sub_category.id}">
                                            <i class="material-icons">edit</i>
                                        </router-link>
        ...
        <div class="btn-create-new">
            <router-link class="item-tool" :to="{path: '/post-category/create'}">
...
<script>
    ...
    import {getPostCategories, deletePostCategory} from '../../models/post.category'
        ...
        methods: {
            fetch(page = 1) {
                axios.all([
                    ...
                    this.categories = first_response.data.data.post_categories;
            ...
            searchEvent({target}) {
                ...
                getPostCategories({
                    ...
                    this.categories = response.data.data.post_categories;
            ...
            report(id) {
                ...
                confirmDialog('Xác nhận', 'Bạn chắc chắn muốn xóa?', function () {
                    deletePostCategory(id).then(response => {
