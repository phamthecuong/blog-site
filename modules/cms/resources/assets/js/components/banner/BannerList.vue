<template>
    <section class="content">
                                ...
                                <div class="form-group m-0">
                                    <div class="form-line">
                                        ...
                                        <input type="text" class="form-control" name="name" v-on:keyup="searchEvent"
                                               placeholder="Nhập tên banner" value=""/>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="body table-responsive">
                            <table class="table table-striped">
                                ...
                                <tr v-for="banner in banners">
                                    <td>
                                        <img :src="banner.image_medium_url" style="width: 100%"/>
                                    </td>
                                    <td>{{banner.id}}</td>
                                    <td>{{banner.name}}</td>
                                    <td>{{banner.order}}</td>
                                    ...
                                    <td>
                                        <span v-if="banner.status == 1" class="font-bold col-teal">Hoạt động</span>
                                        <span v-else class="font-bold col-pink">Không hoạt động</span>
                                    </td>
                                    <td class="text-right">
                                        <router-link class="item-tool" :to="{path: '/banner/'+banner.id}">
                                            <i class="material-icons">edit</i>
                                        </router-link>
                                        <a href="#" class="item-tool" @click="report(banner.id)">
        ...
        <div class="btn-create-new">
            <router-link class="item-tool" :to="{path: '/banner/create'}">
...
<script>
    ...
    import {getBanners, deleteBanner} from '../../models/banner'
        ...
        methods: {
            fetch(page = 1) {
                axios.all([
                    ...
                    this.banners = first_response.data.data;
            ...
            report(id) {
                ...
                confirmDialog('Xác nhận', 'Bạn chắc chắn muốn xóa?', function () {
                    deleteBanner(id).then(response => {
                        that.banners = that.banners.filter(item => item.id !== id);
                    });
                });
            }
        }
